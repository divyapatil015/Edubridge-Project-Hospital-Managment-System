...
export class DoctorAppointmnetListComponent implements OnInit {
      ...
      .subscribe(
        (res: any) => {
          if (!!res && Array.isArray(res) && res.length > 0) {
            this.apptList = res.filter(
              (item: Appointment) =>
                item?.doctor?.doctorId.toString() === this.doctorId.toString()
            );
            this.apptList.forEach((item: Appointment) => {
              this.service
                .getPatientById(item?.patientId)
                .pipe(take(1))
                .subscribe((resp: any) => {
        ...
        if (!!res && res.length > 0) {
          this.apptList.forEach((app: Appointment) => {
            res.forEach((item: Prescription) => {
              if (
                item?.bookingAppointment?.appointmentId.toString() ===
  ...
  addPrescription(apt: Appointment): void {
    ...
    this.service
      .addPrescription(apt?.appointmentId, body)
      .pipe(take(1))
      .subscribe(
        (res: any) => {
